...
def load_configs_from_run(run_path: str) -> tuple[dict, dict]:
    ...
    try:
        with open(params_path, "r", encoding="utf-8") as f:
            ...
            if not run_args:
                logging.error(
                    f"'{params_path}' に 'cli_arguments' キーが見つかりません。ファイルが破損しているか、古いバージョンです。"
                )
                sys.exit(1)
...
def setup_evaluation_vec_env(run_args: dict, game_cfg: dict) -> VecFrameStack:
    ...
    env_kwargs = dict(
        game_config=game_cfg, density=density, resized_shape=(resized_h, resized_w)
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="[V9.3] 学習済みモデル評価＆録画スクリプト"
    )
    parser.add_argument(
        "--run_path",
        type=str,
    ...
    video_path = os.path.join(
        args.run_path, f"evaluation_video_{run_args.get('density', 'default')}.mp4"
    )
    ...
    try:
        ...
        while not done:
            ...
            render_obs, render_reward, render_done, _, render_info = render_env.step(
                action[0]
            )
        ...
        score_percentage = (
            (total_game_score / max_score) * 100 if max_score > 0 else 0.0
        )
        logging.info("評価完了。")
        logging.info(
            f"最終ゲームスコア: {total_game_score:.0f} / {max_score:.0f} ({score_percentage:.2f}%)"
        )
    ...
    if frames:
        try:
            logging.info(
                f"'{video_path}' に動画を保存中 (フレーム数: {len(frames)})..."
            )
            imageio.mimsave(
                video_path, frames, fps=RhythmGameEnv.metadata["render_fps"]
            )
