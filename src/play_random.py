...
def load_game_config() -> dict:
    ...
    try:
        ...
        logging.error(
            f"設定ファイル (src/rhythm_game/config.json) が見つかりません: {e}"
        )
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        logging.info(
            "'human' モードで環境を正常に作成しました。Pygame ウィンドウを確認してください。"
        )
    ...
    try:
        ...
        while not done and not truncated:
            ...
            if reward != 0:
                logging.info(
                    f"フレーム: {frame_count}, 行動: {action}, 報酬: {reward:.1f}, 現在の合計スコア: {score:.1f}"
                )
        ...
        logging.error(
            f"ランダムエージェントの実行中にエラーが発生しました: {e}", exc_info=True
        )
    finally:
        if "env" in locals():
