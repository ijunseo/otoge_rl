...
def load_configs() -> tuple[dict, dict]:
    ...
    try:
        ...
        logging.error("プロジェクトルートから 'uv run python src/main.py' を実行していることを確認してください。")
        sys.exit(1)
    ...
    if device_arg == "auto":
        try:
            ...
            if cuda_available:
                ...
                logging.warning("インストールされた PyTorch が CUDA 12.4+ と互換性がない (CPU版？) か、NVIDIA ドライバに問題がある可能性があります。")
        ...
        device = device_arg
        cuda_available = "N/A (指定)" if device == "cpu" else str(torch.cuda.is_available())
...
def parse_arguments(ppo_cfg: dict, game_cfg: dict) -> argparse.Namespace:
    ...
    # --- 実行制御 ---
    parser.add_argument("--output_path", type=str, required=True, help="結果 (ログ、モデル、グラフ) を保存するフォルダパス。")
    parser.add_argument("--total_timesteps", type=int, default=1_000_000, help="総学習タイムステップ数。")
    parser.add_argument("--device", type=str, default="auto", choices=["auto", "cuda", "cpu"], help="学習に使用するデバイス。")

    # --- 環境設定 (game_cfg) ---
    parser.add_argument("--n_envs", type=int, default=16, help="並列実行する学習環境の数。")
    parser.add_argument("--n_eval_envs", type=int, default=4, help="[V9.3] 並列実行する評価環境の数。(性能改善のため 1->4 に変更)")
    
    # [V9.6] 修正: create_callbacks で使用する 'n_eval_episodes' が抜けていた (AttributeError 修正)
    parser.add_argument("--n_eval_episodes", type=int, default=5, help="[V9.6] 各評価ステップで実行するエピソード数。(デフォルト: 5)")

    parser.add_argument("--n_stack", type=int, default=4, help="フレームスタック数。")
    parser.add_argument("--resized_shape_h", type=int, default=100, help="リサイズ後の観測の高さ。")
    parser.add_argument("--resized_shape_w", type=int, default=75, help="リサイズ後の観測の幅。")
    ...
    parser.add_argument("--learning_rate", type=float, default=ppo_cfg["learning_rate"], help="学習率。")
    parser.add_argument("--n_steps", type=int, default=ppo_cfg["n_steps"], help="各環境が1回の更新までに収集するステップ数。")
    parser.add_argument("--batch_size", type=int, default=ppo_cfg["batch_size"], help="ミニバッチサイズ。")
    parser.add_argument("--n_epochs", type=int, default=ppo_cfg["n_epochs"], help="各更新でのエポック数。")
    parser.add_argument("--gamma", type=float, default=ppo_cfg["gamma"], help="割引率 (Gamma)。")
    parser.add_argument("--gae_lambda", type=float, default=ppo_cfg["gae_lambda"], help="GAE の Lambda。")
    parser.add_argument("--clip_range", type=float, default=ppo_cfg["clip_range"], help="PPO クリップ範囲。")
    parser.add_argument("--ent_coef", type=float, default=ppo_cfg["ent_coef"], help="エントロピー係数。")
    parser.add_argument("--vf_coef", type=float, default=ppo_cfg["vf_coef"], help="価値関数係数。")
    parser.add_argument("--max_grad_norm", type=float, default=ppo_cfg["max_grad_norm"], help="最大勾配ノルム。")

    return parser.parse_args()

def save_parameters(args: argparse.Namespace, ppo_cfg: dict, game_cfg: dict, device_info: str):
    """
...
def setup_eval_env(args: argparse.Namespace, game_cfg: dict) -> VecFrameStack:
    ...
    eval_env = VecMonitor(eval_env, os.path.join(args.output_path, "logs_eval")) # 評価ログを別フォルダに
    
    eval_env = VecFrameStack(eval_env, n_stack=args.n_stack, channels_order='last')
    return eval_env

def create_model(env: VecFrameStack, args: argparse.Namespace, ppo_cfg: dict, device: str) -> PPO:
    """
    [V8.1] PPO モデルを生成します。
    """
    ...
    logging.info(f"PPO モデル ({ppo_params['policy']}) をデバイス '{device}' 上に作成しました。")
...
def create_callbacks(args: argparse.Namespace, eval_env: VecFrameStack) -> CallbackList:
    ...
    checkpoint_callback = CheckpointCallback(
      save_freq=max(eval_freq_steps * 10, eval_freq_steps), # 10 ロールアウトごとにチェックポイント
...
def save_plots(args: argparse.Namespace):
    ...
    try:
        ...
        if not os.path.exists(monitor_path):
             ...
             if not os.path.exists(monitor_path):
                 logging.warning(f"Monitor.csv が見つかりません: {monitor_path}。学習リワードグラフはスキップされます。")
                 monitor_data = None
        ...
        if os.path.exists(monitor_path):
            ...
            try:
                monitor_data = np.loadtxt(monitor_path, delimiter=",", skiprows=2, usecols=(0, 2)) # reward, time
                ...
                logging.warning(f"Monitor.csv の読み込みに失敗しました ({e})。学習リワードグラフはスキップされます。")
                monitor_data = None
        
        # 評価精度 (evaluations.npz)
        eval_path = os.path.join(model_path, "evaluations.npz")
        if not os.path.exists(eval_path):
            logging.warning(f"evaluations.npz が見つかりません: {eval_path}。評価精度グラフはスキップされます。")
            eval_data = None
        else:
        ...
        if 'monitor_data_rewards' in locals() and monitor_data_rewards is not None:
            ...
            axs[0].plot(monitor_data_timesteps, monitor_data_rewards, alpha=0.5, label='Reward (Raw)')
            # トレンドライン (移動平均)
            if len(monitor_data_rewards) > 100:
                moving_avg = np.convolve(monitor_data_rewards, np.ones(100)/100, mode='valid')
                # 이동 평균의 x축도 time 에 맞춰 조정
                avg_timesteps = monitor_data_timesteps[len(monitor_data_timesteps) - len(moving_avg):]
                axs[0].plot(avg_timesteps, moving_avg, color='r', label='Moving Avg (100 ep)')
        ...
        # サブプロット 2: 評価精度
        if eval_data:
            axs[1].plot(eval_data['timesteps'], eval_data['results'][:, 0]) # Mean Reward
        ...
        import traceback
        logging.error(traceback.format_exc()) # V9.6: グラフエラーの詳細なトレースバック
...
def main():
    ...
    try:
        # 1. セットアップ
        setup_logging()
        logging.info(f"--- [V9.6] Otoge RL Agent 学習開始 ---") # バージョンを V9.6 に更新
        ...
        logging.error("V9.2 計画に基づき、'pyproject.toml' に 'tqdm' と 'rich' を追加し、")
        ...
        if model is not None:
            final_model_path = os.path.join(args.output_path, "models", "final_model.zip")
