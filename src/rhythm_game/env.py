...
class RhythmGameEnv(gym.Env):
    ...
    def __init__(
        self, game_config: dict, density: str = "medium", render_mode: str | None = None
    ):
        """
        環境を初期化します。
    ...
    def render(self):
        ...
        # 観測用の描画 (判定ラインなし)
        obs_surface = pygame.surfarray.make_surface(
            np.transpose(self._get_obs().repeat(3, axis=2), (1, 0, 2))
        )
